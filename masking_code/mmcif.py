""" From Boltz training code. I made some hacks to read cif and pdb files with nonideal formatting. - Dru
"""
...
def parse_mmcif(  # noqa: C901, PLR0915, PLR0912
    ...
    '''
    deposit_date, release_date, revision_date = 0, 0, 0
    resolution = 1
    method = 'foo'
    ...
    structure.make_mmcif_document().write_file('dump.cif') 
    # Parse chains
    chains: list[ParsedChain] = []
    chain_seqs = []
    for raw_chain in structure[0].subchains():
        ...
        if entity_type == "Polymer":
            ...
            if entity.full_sequence == []:
                entity.full_sequence = [res.name for res in structure[0][subchain_id.split('x')[0]].get_polymer()]
